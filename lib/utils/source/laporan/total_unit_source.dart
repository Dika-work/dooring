...
class TotalUnitSource extends DataGridSource {
  final int selectedYear;
  final List<TotalUnitModel> totalUnitModel;

  TotalUnitSource({
    required this.selectedYear,
    required this.totalUnitModel,
  }) {
    _buildData();
  }
  ...
  void _buildData() {
    ...
    for (var data in totalUnitModel) {
      if (data.wilayah == "SAMARINDA") {
        samarindaResults[data.bulan] = data.totalUnit;
      } else if (data.wilayah == "MAKASSAR") {
        makassarResults[data.bulan] = data.totalUnit;
      } else if (data.wilayah == "PONTIANAK") {
        pontianakResults[data.bulan] = data.totalUnit;
      } else if (data.wilayah == "BANJARMASIN") {
        banjarMasinResults[data.bulan] = data.totalUnit;
      }
    }
